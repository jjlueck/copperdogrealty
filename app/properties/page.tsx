    ...
    <Card className="overflow-hidden hover:shadow-xl transition-all group border-0 shadow-md flex flex-col h-full">
      <div className="relative aspect-[4/3] bg-muted group/image">
        ...
        <div className="absolute top-3 left-3 right-3 flex items-center justify-between z-10 pointer-events-none">
          ...
          <button className="w-8 h-8 bg-white/80 hover:bg-white rounded-full flex items-center justify-center transition-colors shadow-sm text-muted-foreground hover:text-red-500 pointer-events-auto">
...
export default function PropertiesPage() {
  ...
  // Debounce helper or just fetch on button click? 
  // For simplicity and API quota, let's fetch on mount and on "Apply Filters" click.
  ...
  const fetchProperties = async () => {
    ...
    try {
      ...
      params.append("limit", "50") // Fetch up to 50 listings
          ...
          <div className="container mx-auto px-4">
            ...
            {error && (
              <div className="text-center py-20 text-red-500 bg-red-50 rounded-lg border border-red-100 p-8">
                ...
                <Button variant="outline" className="mt-4" onClick={() => fetchProperties()}>Try Again</Button>
              </div>
            )}
            ...
            {!loading && !error && properties.length === 0 && (
              <div className="text-center py-20 text-muted-foreground bg-white rounded-lg border border-border p-8 shadow-sm">
                ...
                <Button variant="link" onClick={() => setFilters({city: "", minPrice: "", maxPrice: "", minBeds: "", minBaths: ""})} className="mt-2">
                  Clear all filters
                </Button>
              </div>
            )}
